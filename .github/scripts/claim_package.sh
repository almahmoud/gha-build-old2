...
if [ ! -s tobuild.txt ]; then
      grep -Pzo "(?s)\s*\"\N*\":\s*\[\s*\]" packages.json | awk -F'"' '{print $2}' | grep -v '^$' | xargs -i bash -c 'touch lists/{} && if ! [ -s "lists/failed/{}" ]; then if ! grep -i ".tar.gz$" lists/{}; then if [ $(cat lists/{} | wc -l) = "1" ]; then echo "readytobuild" > lists/{}; fi; fi; fi'
