...
if [ ! -s tobuild.txt ]; then
      grep -Pzo "(?s)\s*\"\N*\":\s*\[\s*\]" packages.json | awk -F'"' '{print $2}' | grep -v '^$' | xargs -i bash -c 'touch lists/{} && echo "{}" && if ! [ -s lists/failed/{} ]; then if ! grep -i "tar" lists/{}  ; then echo "readytobuild" > lists/{}; fi; fi'
