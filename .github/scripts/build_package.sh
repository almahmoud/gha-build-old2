...
if [ $CLONED = 0 ]; then Rscript -e "p <- .libPaths(); p <- c('$LIBRARY', p); .libPaths(p); if(install.packages('./$PKG', INSTALL_opts = '--build', update = TRUE, quiet = FALSE, force = TRUE, keep_outputs = TRUE, type = 'source') %in% rownames(installed.packages())) q(status = 0) else q(status = 1)"; fi
cp *.tar.gz /tmp/tars/
cp *.out /tmp/logs/
